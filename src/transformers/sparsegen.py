# Inherited and modified from https://github.com/anirbanl/sparsegen/blob/master/nlg/probability.py
...
class Sparsegen_lin(torch.nn.Module):
    def __init__(self, lam, data_driven=False, normalized=True, z_axis=-1):
    ...
    # Note: modified to support tensor 
    def forward(self, input):
        bs = input.data.size()[0]
        dim = input.data.size()[-1]
        ...
        zdim = input.shape[self.z_axis]
        input = input.transpose(0, self.z_axis)
        cached_shape = input.shape
        z = input.reshape(zdim, -1).transpose(0, 1)

        # z = input.transpose(0, -1) # assume last axis is the dimension of z 
        # z = input.type(dtype)
        ...
        # TODO: revise below, ignored in first adaptation
        if self.data_driven:
        ...
        #calculate k(z)
        z_cumsum = torch.cumsum(z_sorted, dim=1)
        k = Variable(torch.arange(1, dim + 1).unsqueeze(0).repeat(z.shape[0],1)).to(z.device)
        ...
        prob = z.sub(tau_z.view(z.shape[0],1).repeat(1, zdim)).clamp(min=0)
        if self.normalized:
               prob /= (1-self.lam)

        prob = prob.transpose(0, 1).reshape(cached_shape).transpose(0, self.z_axis)
        return prob
